...
function processData(data) {
  ...
  if (data.operatorInfo) setOperatorInfo(data.operatorInfo);
    console.log(data.action + " 5");

  // return;  

  if (data.hardLimit) setHardLimit(data.hardLimit);
  // return;
  if (data.cryptomatModel) setCryptomatModel(data.cryptomatModel); ///----Problem 
  // return; //- P
...
function setExchangeRate(_rates) {
  ...
  if (rates.cashIn) {
    ...
    var rateStr = formatFiat(cryptoToFiat.precision(2).toNumber(), 2);
  ...
  if (rates.cashOut) {
    ...
    var cashOutCryptoToFiat = cashOut && formatFiat(cashOut.precision(2).toNumber(), 2);
