...
$(function () {
  ...
  var wifiKeyboard = new Keyboard('wifi-keyboard').init();
  ...
  Array.from(document.getElementsByClassName('viewport')).forEach(function (it) {
    var possibleName = it.className.split(/\s+/).filter(function (it) {
      return it.endsWith('_state');
    });

    if (!possibleName || !possibleName.length) return console.log(possibleName);

    var name = possibleName[0];
    var types = null;
    if (it.dataset.screentype) {
      var _finalTypes;

      types = it.dataset.screentype.split(/\s+/);
      (_finalTypes = finalTypes).push.apply(_finalTypes, _toConsumableArray(types));
    }

    screens.push({ name: name, types: types });
  });
...
function setupFakes() {
  var amount = ['<span class="integer">34</span><span class="decimal-char">', '.', '</span><span class="decimal">479</span>'].join('');

  var address = 'wjy98nu928ud1o82dbj2u9i81wqjjyu98iwn';
  ...
  var states = [$('.scan_id_photo_state'), $('.scan_id_data_state'), $('.security_code_state'), $('.register_phone_state'), $('.register_us_ssn_state'), $('.terms_screen_state'), $('.verifying_id_photo_state'), $('.verifying_face_photo_state'), $('.verifying_id_data_state'), $('.sms_verification_state'), $('.permission_id_state'), $('.bad_phone_number_state'), $('.bad_security_code_state'), $('.max_phone_retries_state'), $('.failed_permission_id_state'), $('.failed_verifying_id_photo_state'), $('.blocked_customer_state'), $('.fiat_error_state'), $('.fiat_transaction_error_state'), $('.failed_scan_id_data_state'), $('.sanctions_failure_state'), $('.error_permission_id_state'), $('.scan_face_photo_state'), $('.retry_scan_face_photo_state'), $('.permission_face_photo_state'), $('.us_ssn_permission_state'), $('.failed_scan_face_photo_state'), $('.hard_limit_reached_state'), $('.failed_scan_id_photo_state'), $('.retry_permission_id_state'), $('.waiting_state')];
  ...
  ['.reverse-exchange-rate', '.total-crypto-rec'].forEach(function (it) {
    updateCrypto(it);
  });
...
function qrize(text, target, color, lightning) {
  ...
  function doTransition() {
    ...
    var target = document.getElementById('clicker-insert_bills_state');
    tl.set('.fade-in-delay', { opacity: 0, y: +30 }).set('.fade-in', { opacity: 0, y: +30 }).set('.crypto-buttons', { zIndex: -2 }).to(background, 0.5, { scale: 2 }).to('.fade-in', 0.4, {
      opacity: 1,
      onStart: click,
      onStartParams: [{ target: target }],
      y: 0
    }, '=-0.2').to('.fade-in-delay', 0.4, { opacity: 1, y: 0 }, '=-0.2').set(background, { scale: 1 }).set('.crypto-buttons', { zIndex: 0 });
  }
