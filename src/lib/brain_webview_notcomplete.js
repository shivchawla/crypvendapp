...
Brain.prototype.run = async function run(webview) {
  console.log('Run: crypto Machine software initialized.')
  const self = this
  this._init()
  this.webview = webview;

  actionEmitter.on('browserEvent', (event) => {
      console.log("Brain.js: Launching browser Event");
      console.log(event);
      // console.log(this.webview);

      if (this.webview) {
          const js = `console.log("Emitting event"); window.dispatchEvent(new MessageEvent('message', {data: ${JSON.stringify(event)}}))`;
            console.log(js);
  ...
  this.webview.onMessage = (syntheticEvent) => {
    ...
    this.processBrowserEvent(JSON.parse(nativeEvent.data));
  }

  console.log("Run: Webview initialized. Calling _connectedBrowser");
  this._connectedBrowser();

  const callback = function () {
    self._transitionState('restart')
    console.log('Run: Scheduled restart after idle time.')
    // process.exit()
    // RNRestart.Restart();
  ...
  if (!this.hasFrontFacingCamera) console.log('Run: Warning: no front facing camera detected.')
...
Brain.prototype.initialize = async function initialize () {
  ...
  const clientCert = await pairing.init(this.certPath);
...
Brain.prototype._init = function init () {
  // this._initHearbeat() -- file socket server -- not required in POS
  // this._initWifiEvents(); //-- WIFI Events not required in POS
  // this._initBrowserEvents()
  // this._initBillValidatorEvents() -- Not reuired in POS
...
// Brain.prototype.attachBrowserEvent = function attachBrowserEvent() {
//     actionEmitter.on('browserEvent', (event) => {
//       console.log("App.js: browserEvent");
//       console.log(event);
//       // console.log(this.webview);
//       if (this.webview) {
//           const js = `console.log(${JSON.stringify(event)}); console.log("Emitting event"); document.dispatchEvent(new MessageEvent('message', {data: ${JSON.stringify(event)}}))`;
//             console.log(js);
//         this.webview.injectJavaScript(js);
//       } else {
//         console.log("Webview is null");
...
Brain.prototype._initActionEvents = function _initActionEvents () {
  ...
  //Not needed as processBrainActions only does Bay lights
...
//There are no bay lights in case of POS --- then what?
...
Brain.prototype._wifiList = function _wifiList () {
  ...
  browserEmit({action: 'wifiList'})
}

Brain.prototype._wifiPass = function _wifiPass (data) {
  browserEmit({action: 'wifiPass', wifiSsid: data})
...
Brain.prototype._pairingScan = function _pairingScan () {
  ...
  browserEmit({action: 'pairingScan'})
...
Brain.prototype._idle = function _idle (locale) {
  ...
  // console.log("Diableing Bill Validator");
...
Brain.prototype._start = function _start () {
  ...
  const updateRec = {
    ...
    minimumTx: 100, //this.billValidator.lowestBill(coin.minimumTx),
...
Brain.prototype.commitCashOutTx = function commitCashOutTx () {
  return this.updateTx({})
    .then(tx => {
      ...
      return browserEmit({depositInfo})
    })
...
Brain.prototype._batchDispense = function _batchDispense (notesToDispense, { batchAmount, dispenseRecords, currentBatch, error }) {
  ...
  browserEmit({ action: 'dispensing', dispenseBatch: { current: currentBatch, of: batchAmount } })
  ...
  return this.billDispenser.dispense(notesToDispense, currentBatch, batchAmount)
    .then(({ value, error }) => {
      ...
      browserEmit({ action: screen, dispenseBatch: { current: currentBatch, of: batchAmount } })
